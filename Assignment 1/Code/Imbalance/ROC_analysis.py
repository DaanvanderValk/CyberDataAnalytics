...
if __name__ == "__main__":
    ...
    for line_ah in ah:
        ...
        # Data processing: we convert the amount based on the currency to EUR.
        # The dataset contains dates records from 01/07/2015 to 31/10/2015,
        # so for simplification, we use the exchange rates from 31/08/2015.
        # Source: https://x-rates.com/historical/?from=EUR&amount=1&date=2015-08-31
        if currencycode == 'AUD':
            amount_in_eur = amount * 0.634006
        if currencycode == 'GBP':
            amount_in_eur = amount * 1.372043
        if currencycode == 'MXN':
            amount_in_eur = amount * 0.053364
        if currencycode == 'NZD':
            amount_in_eur = amount * 0.566923
        if currencycode == 'SEK':
            amount_in_eur = amount * 0.105393
        # The amount_in_eur is passed to the classifier instead of amount
